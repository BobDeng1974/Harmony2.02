                          ...
                          const uint32_t idx)
{
    ...
    /*memcpy(((uint8_t*)&color),
    buf_ptr,
    GFX_ColorInfo[buffer->mode].bpp >> 3);*/
                                    ...
                                    const GFX_Point* const p)
{
    ...
    for(i = 0; i < GFX_ColorInfo[buffer->mode].size; i++)
        ((uint8_t*)&color)[i] = offs_ptr[i]; 
                                  ...
                                  GFX_PixelBuffer* out)
{
    ...
    // nothing to do if the desired rectangle is not inside the buffer rectangle
    if(GFX_RectIntersects(&buffer_rect, rect) == GFX_FALSE)
        return GFX_FAILURE;

    // trim the rectangle if it intersects the edges of the buffer rectangle
    GFX_RectClip(&buffer_rect, rect, &clip_rect);

    return GFX_PixelBufferAreaGet_Unsafe(buffer, &clip_rect, mem_intf, out);
}
                                         ...
                                         GFX_PixelBuffer* out)
{
    ...
    for(row = 0; row < rect->height; row++)
    {
        ...
        mem_intf->memcpy(((uint8_t*)out->pixels) + (row * out->size.width),
                         buf_ptr,
                         rect->width);
    }
                                     ...
                                     GFX_Color color)
{
    ...
    source_ptr = (uint8_t*)&color;

    // get address of dest pixel
    dest_ptr = (uint8_t*)GFX_PixelBufferOffsetGet_Unsafe(buffer, pnt);
    
    // copy the pixel value
    //mem_intf->memcpy(dest_ptr, source_ptr, (GFX_ColorInfo[buffer->mode].bpp >> 3));
    
	// array copy method, not sure if this is faster than a memset call per pixel
    for(i = 0; i < (GFX_ColorInfo[buffer->mode].bpp >> 3); i++)

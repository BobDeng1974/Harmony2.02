...
void _laButtonWidget_Constructor(laButtonWidget* btn)
{
	...
	//btn->widget.update = &(laWidget_Update_FnPtr)_laButtonWidget_Update;
...
void laButtonWidget_SetToggleable(laButtonWidget* btn, laBool toggleable)
{
	if(btn == NULL)
		return;
...
laBool laButtonWidget_GetPressed(laButtonWidget* btn)
{
	...
	return btn->state == LA_BUTTON_STATE_DOWN;
}
...
void laButtonWidget_SetPressed(laButtonWidget* btn, laBool pressed)
{
    ...
    if(btn->pressedOffset != 0 &&
       (btn->pressedImage != NULL ||
        (btn->text.table_index != LA_STRING_NULLIDX || btn->text.length > 0)))
        dirty = LA_TRUE;
...
/*static void cacheTextRect(laButtonWidget* btn)
{

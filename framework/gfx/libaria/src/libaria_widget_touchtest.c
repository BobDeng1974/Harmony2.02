...
void _laTouchTestWidget_Constructor(laTouchTestWidget* tch)
{
	...
	tch->widget.drawBackground = LA_TRUE;
...
laResult laTouchTest_AddPoint(laTouchTestWidget* tch, GFX_Point* pnt)
{
    ...
    laWidget_Invalidate((laWidget*)tch);
							            ...
							            laInput_TouchMovedEvent* evt)
{
	...
	laUtils_ClipRectToParent(tch->widget.parent, &rect);
	laUtils_RectToScreenSpace(tch->widget.parent, &rect);

...
//
static void bw_conversion(char *ptr, uint32_t *values)
{
    ...
    { 
        ...
        values[0] = bw1;
        values[1] = bw2;
        printf("\nGiven in BW: %d+%d=%d",bw1,bw2,bw1+bw2);
    }
}
        ...
        case UDP_PROTOCOL:
            ...
            for(i = 0, mpIState = gIperfState; i < nIperfSessions; i++, mpIState++)
    	     {            
                ...
                {
                //printf("%d(wait:%d), mpt:%d  ",pIState->mTypeOfService,pIState->sockWaitToSend, mpIState->mTypeOfService);
                ...
                switch(pIState->mTypeOfService)
                {
                    ...
                    case TCPIP_IPV4_RELIABILITY_HIGH:  /*0xA0 Video, check for only Voice*/
                        if(mpIState->mTypeOfService == TCPIP_IPV4_THROUGHPUT_HIGH)
                            ...
                            {
                                mpIState->waitCount += 1;
                                //printf("%d, ",mpIState->sockWaitToSend);
                                return IPERF_TX_WAIT;
                            }
                        break;

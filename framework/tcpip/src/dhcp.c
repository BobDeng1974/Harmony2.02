...
static __inline__ void __attribute__((always_inline)) _DHCPSetFailTimeout(DHCP_CLIENT_VARS* pClient, bool resetTmo, bool isRunTime)
{
    ...
    if(resetTmo || pClient->tOpStart == 0)
    {
        ...
        SYS_CONSOLE_PRINT("DHCP set fail tmo: %d at: %s\r\n", pClient->tOpStart, isRunTime ? "run" : "init");
#endif  // TCPIP_DHCP_DEBUG_MASK
    }
}
...
static void _DHCPDbgStatus(DHCP_CLIENT_VARS* pClient)
{
    ...
    {
        SYS_CONSOLE_PRINT("DHCP cli: %d, state: %s\r\n", DHCPClients - pClient, _DHCPStatTbl[pClient->smState]);
...
static void _DHCPDbgAddServiceEvent(DHCP_CLIENT_VARS* pClient, TCPIP_STACK_ADDRESS_SERVICE_EVENT addEv, const char* extraMsg)
{
    ...
    SYS_CONSOLE_PRINT("DHCP cli: %d, address event: %s, extra: %s, time: %d\r\n", DHCPClients - pClient, evMsg, extraMsg ? extraMsg : "none", _DHCPSecondCountGet());
}
...
static void _DHCPDbgRxDisplay(DHCP_CLIENT_VARS* pClient, unsigned int msgType)
{
    const char* dhcpMsg;
    switch(msgType)
    {
        case TCPIP_DHCP_UNKNOWN_MESSAGE:
            dhcpMsg = "unkn";
            break;

        case TCPIP_DHCP_OFFER_MESSAGE:
            dhcpMsg = "offer";
            break;
    ...
    SYS_CONSOLE_PRINT("DHCP cli: %d, rx: %s\r\n", DHCPClients - pClient, dhcpMsg);
}

#else
#define _DHCPDbgRxDisplay(pClient, msgType)
#endif  // TCPIP_DHCP_DEBUG_MASK
...
bool TCPIP_DHCP_Initialize(const TCPIP_STACK_MODULE_CTRL* const stackCtrl, const TCPIP_DHCP_MODULE_CONFIG* pDhcpConfig)
{
    ...
    {   // use a safe value
        pClient->tOpFailTmo = TCPIP_DHCP_INIT_FAIL_TMO;
    }
    else
    {
        pClient->tOpFailTmo = (uint16_t)pDhcpConfig->dhcpTmo;
    }
    ...
    for(netIx = 0; netIx < nNets; netIx++) 
    {
        ...
        switch(pClient->smState)
        {
            ...
            case TCPIP_DHCP_WAIT_LINK:
                ...
                _DHCPClientStateSet(pClient, TCPIP_DHCP_GET_SOCKET);
                // and fall through
            ...
            case TCPIP_DHCP_BOUND:
                ...
                // time for renew
                _DHCPClientStateSet(pClient, TCPIP_DHCP_BOUND_RENEW);
                // No break
...
minimum packet that needs to be accepted: 236 + 312 = 548 bytes (RFC)
	...
	********************************************************************/
	...
	TCPIP_UDP_Discard(s);                     
    _DHCPDbgRxDisplay(pClient, msgType);
	return msgType;
}
...
static void _DHCPNotifyClients(TCPIP_NET_IF* pNetIf, TCPIP_DHCP_EVENT_TYPE evType)
{
    ...
    {
        uint32_t evTime = _DHCPSecondCountGet();
        SYS_CONSOLE_PRINT("DHCPc%d connection: %s, t: %d\r\n", TCPIP_STACK_NetIndexGet(pNetIf), message, evTime);
    }

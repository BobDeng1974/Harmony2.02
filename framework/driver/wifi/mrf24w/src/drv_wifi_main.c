...
static TCPIP_MAC_HANDLE MRF24W_MACOpen(const SYS_MODULE_INDEX drvIndex, const DRV_IO_INTENT intent);
...
static TCPIP_MAC_RES MRF24W_MACParametersGet(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_PARAMETERS *pMacParams);
static TCPIP_MAC_RES MRF24W_MACRegisterStatisticsGet(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_STATISTICS_REG_ENTRY *pRegEntries,
        int nEntries, int *pHwEntries);
static size_t MRF24W_MACGetConfig(TCPIP_MAC_HANDLE hMac, void *configBuff, size_t buffSize, size_t *pConfigSize);
...
const TCPIP_MAC_OBJECT DRV_MRF24W_MACObject =
{
...
{
	{
		&DRV_MRF24W_MACObject,		// specific PIC32 MAC data
		NULL,						// pNetIf
		false,						// isInit
		false,						// isOpen
...
static SYS_MODULE_OBJ MRF24W_MACInitialize(const SYS_MODULE_INDEX index, const SYS_MODULE_INIT *const init)
{
    ...
    if (s_pic32_mrf24w_mac_dcpt->isInit)
        return (SYS_MODULE_OBJ)s_pic32_mrf24w_mac_dcpt;

    // no other type supported
    if (stackData->moduleId != TCPIP_MODULE_MAC_MRF24W)
        return SYS_MODULE_OBJ_INVALID;

    if (MRF24W_MACEventInit(s_pic32_mrf24w_mac_dcpt, stackData->eventF,
            stackData->eventParam, WF_EVENT_IPL, WF_EVENT_SIPL) != TCPIP_MAC_RES_OK)
        return SYS_MODULE_OBJ_INVALID;
    ...
    if (!DRV_WIFI_AllTasksCreate((void *)s_pic32_mrf24w_mac_dcpt, NULL, NULL))
        return SYS_MODULE_OBJ_INVALID; // create all Wi-Fi tasks
#endif

    s_content_stackData = *stackData; // save pointer to stack data
    s_pic32_mrf24w_mac_dcpt->isInit = true;
    s_pic32_mrf24w_mac_dcpt->sysStat = SYS_STATUS_BUSY; // more init works are waiting, so set it to busy
    return (SYS_MODULE_OBJ)s_pic32_mrf24w_mac_dcpt;
}
...
static void MRF24W_MACTasks(SYS_MODULE_OBJ object)
{
    ...
    case SYS_STATUS_BUSY:
        ...
        if (res == TCPIP_MAC_RES_PENDING) {
            ...
            SYS_CONSOLE_MESSAGE("MRF24W initialization failed\r\n");
            break;
        }
#endif
        ...
        if (res == TCPIP_MAC_RES_PENDING) {
            ...
            MRF24W_MACDeinitialize((SYS_MODULE_OBJ)pDcpt);
            pDcpt->sysStat = SYS_STATUS_ERROR;
            SYS_CONSOLE_MESSAGE("MRF24W initialization failed\r\n");
        }
        break;
...
static void MRF24W_MACDeinitialize(SYS_MODULE_OBJ object)
{
    _MRF24W_MACDeinitialize((MRF24W_MAC_DCPT *)object);
}
...
static SYS_STATUS MRF24W_MACStatus(SYS_MODULE_OBJ object)
{
    return ((MRF24W_MAC_DCPT *)object)->sysStat;
}
...
static TCPIP_MAC_HANDLE MRF24W_MACOpen(const SYS_MODULE_INDEX drvIndex, const DRV_IO_INTENT intent)
{
    if (!s_pic32_mrf24w_mac_dcpt->isOpen) {
        s_pic32_mrf24w_mac_dcpt->isOpen = true;
        MulticastFilter_Initialize();
    }
    return (TCPIP_MAC_HANDLE)s_pic32_mrf24w_mac_dcpt;
}
...
static TCPIP_MAC_RES MRF24W_MACRegisterStatisticsGet(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_STATISTICS_REG_ENTRY* pRegEntries, int nEntries, int* pHwEntries)
{
    /* unsupported */
    return TCPIP_MAC_RES_OP_ERR;
}
...
{
    return s_pic32_mrf24w_mac_dcpt->isInit;
}

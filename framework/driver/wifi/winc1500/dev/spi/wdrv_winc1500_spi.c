/*******************************************************************************
  ...
  Summary:
    WINC1500 Wi-Fi Driver SPI Communication Support
...
#define SPI_WAIT_FOR_TX_COMPLETION() WDRV_SemTake(&s_dmaTxSync, OSAL_WAIT_FOREVER)
#define SPI_WAIT_FOR_RX_COMPLETION() WDRV_SemTake(&s_dmaRxSync, OSAL_WAIT_FOREVER)
...
{
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, WDRV_SPI_SSN_PORT_CHANNEL, WDRV_SPI_SSN_BIT_POS);
}
...
static bool Spi_Init(void)
{
    ...
    WDRV_SemInit(&s_dmaTxSync);
    WDRV_SemInit(&s_dmaRxSync);
...
{
    WDRV_SemDeInit(&s_dmaTxSync);
    WDRV_SemDeInit(&s_dmaRxSync);
...
bool WDRV_SPI_Out(unsigned char *const buf, uint32_t size)
{
    bool ret = true;
    int c = 0;
...
bool WDRV_SPI_In(unsigned char *const buf, uint32_t size)
{
    bool ret = true;
    int c = 0;

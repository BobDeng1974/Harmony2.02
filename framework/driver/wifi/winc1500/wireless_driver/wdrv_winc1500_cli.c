...
#define check_module_up() if (!g_wdrv_priv.isDriverOpen) \
	{SYS_CONSOLE_MESSAGE("WINC1500 is down - command cannot work\r\n"); return false;}
...
{
    *p_state = WDRV_ConnectionStatus_Get();
}
...
{
    if ((len >= 8) && (len <= 63)) {
        WDRV_CONFIG_PARAMS(securityMode) = WDRV_SECURITY_WPA_AUTO_WITH_PASS_PHRASE;
        WDRV_CONFIG_PARAMS(securityKeyLen) = len;
        memcpy(WDRV_CONFIG_PARAMS(securityKey), key, len);
    } else {
...
{
    if (argc < 2) {
        SYS_CONSOLE_MESSAGE("Missing URL. Type, for instance, ota http://192.168.0.4/winc1500_ota.bin\r\n");
        return WDRV_ERROR;
    }
    ...
    g_wdrv_priv.isFwUpgradeRequested = true;
    WDRV_ASSERT(strlen(argv[1]) <= sizeof(g_wdrv_priv.FwServerUrl) - 1, "URL is too long");
    strcpy((void *)g_wdrv_priv.FwServerUrl, argv[1]);

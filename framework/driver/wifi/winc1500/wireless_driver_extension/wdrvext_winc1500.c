/*******************************************************************************
  Wireless driver WINC1500 extension.
...
static WINC1500_SCAN_CONTROL s_scanControl;
...
int32_t WDRV_EXT_Initialize(WDRV_HOOKS const *const hooks)
{
    int32_t res = 0;
    ...
    WDRV_SemInit(&g_wdrvext_priv.scanResultWait);
    WDRV_SemInit(&g_wdrvext_priv.connInfoWait);
    WDRV_SemInit(&g_wdrvext_priv.eventWait);

    s_winc1500_intf.FrameReceived_CB = hooks->ind.frameReceived;
    s_winc1500_intf.ConnectEvent_CB = hooks->ind.connectEvent;
    s_winc1500_intf.RFReady_CB = hooks->ind.rfReady;
    s_winc1500_intf.ScanDone_CB = hooks->ind.scanCompleted;
    s_winc1500_intf.InitDone_CB = hooks->ind.initCompleted;
    s_winc1500_intf.WPSDone_CB = hooks->ind.wpsCompleted;
    s_winc1500_intf.get_rx_buf = hooks->get_rx_buf;
    if (hooks->isFwUpgradeRequested) {
        s_winc1500_intf.ethMode = false;
        s_winc1500_intf.FwServerUrl = hooks->FwServerUrl;
    } else {
...
#else /* !FW_UPDATE_OVER_SERIAL_ENABLED */
...
#endif /* FW_UPDATE_OVER_SERIAL_ENABLED */
...
void WDRV_EXT_Deinitialize(void)
{
    ...
    WDRV_SemGive(&g_wdrvext_priv.eventWait);

    while (g_wdrvext_priv.deinit_in_progress)
        WDRV_TIME_DELAY(10);

    WDRV_SemDeInit(&g_wdrvext_priv.eventWait);
    WDRV_SemDeInit(&g_wdrvext_priv.scanResultWait);
    WDRV_SemDeInit(&g_wdrvext_priv.connInfoWait);
...
uint32_t WDRV_EXT_CmdConnectContextBssidGet(uint8_t *bssId)
{
    ...
    if (ret == 0) {
    	WDRV_SemTake(&g_wdrvext_priv.connInfoWait, OSAL_WAIT_FOREVER);
...
void WDRV_EXT_CmdSecWEPSet(uint8_t *key, uint16_t len)
{
	...
	s_connData.keyIdx = 1; /* 1 is chosen per default. This is configrable. */
	memcpy(s_connData.key, key, s_connData.keyLen);
...
void WDRV_EXT_ScanResultGet(uint8_t listIndex, WDRV_SCAN_RESULT *p_scanResult)
{
    ...
    WDRV_SemTake(&g_wdrvext_priv.scanResultWait, OSAL_WAIT_FOREVER);

...
)
{
    ...
    if (sramInit->registerWithFs)
    {
        ...
        SYS_FS_MEDIA_MANAGER_Register(drvIndex, drvIndex, &sramMediaFunctions, SYS_FS_MEDIA_TYPE_RAM);
    }
...
)
{
    ...
    _DRV_SRAM_VALIDATE_EXPR((NULL == DRV_SRAM_ValidateClientHandle(handle)), DRV_SRAM_COMMAND_HANDLE_INVALID);

...
static void _DRV_MIIM_SMIClockSet(ETH_MODULE_ID ethphyId, uint32_t hostClock, uint32_t maxMIIMClock );
...
void PLIB_ETH_ClearDataValid( ETH_MODULE_ID index )
{    
...
static void _DRV_MIIM_ProcessOp( DRV_MIIM_OBJ * pMiimObj, DRV_MIIM_OP_DCPT* pOpDcpt)
{
    ...
    ETH_MODULE_ID ethphyId = pMiimObj->ethphyId;
    ...
    switch(pOpDcpt->opStat)
    {
        case DRV_MIIM_TXFER_START:
            ...
            {   // scan
                ...
                newStat = DRV_MIIM_TXFER_SCAN_STALE;
            }
        ...
        case DRV_MIIM_TXFER_RDWR_WAIT_COMPLETE:
            ...
            if(pOpDcpt->opType == DRV_MIIM_OP_READ)
            {
                ...
                pOpDcpt->opData = PLIB_ETH_MIIMReadDataGet(ethphyId); // get the read register
            }
        ...
        case DRV_MIIM_TXFER_SCAN_VALID:
            if(!PLIB_ETH_DataNotValid(ethphyId))
            {   // there's data available
                pOpDcpt->opData = PLIB_ETH_MIIMReadDataGet(ethphyId);
                PLIB_ETH_ClearDataValid(ethphyId);
...
static void _DRV_MIIM_ReleaseOpDcpt(DRV_MIIM_OBJ* pMiimObj, DRV_MIIM_OP_DCPT* pOpDcpt, SINGLE_LIST* pRemList, DRV_MIIM_QUEUE_TYPE qType)
{
    ...
    if(wasScan)
    {
        PLIB_ETH_MIIMScanModeDisable(pMiimObj->ethphyId);
        pMiimObj->objFlags &= ~DRV_MIIM_OBJ_FLAG_IS_SCANNING;
    }

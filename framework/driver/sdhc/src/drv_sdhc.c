...
)
{
    ...
    if (((csdPtr[14] >> 6) & 0x03) == 0x01)
    {
        ...
        dObj->cardCtxt->discCapacity = ((uint32_t)(cSize + 1) * (uint32_t)(1024));
    }
    else
    {
        /* CSD Version 1.0 */
        ...
        dObj->cardCtxt->discCapacity = (((uint32_t)(cSize + 1) * mult) * blockLength);
    }
}
        ...
        case DRV_SDHC_INIT_PRE_SCR_STATE:
            {
                ...
                sdhostSetupDma (&dObj->cardCtxt->scr[0], 8);
        ...
        case DRV_SDHC_INIT_PRE_SWITCH_CMD:
            {
                ...
                /* Set up the DMA for the data transfer. */
                sdhostSetupDma (&dObj->cardCtxt->switchStatus[0], 64);
...
)
{
    ...
    switch (dObj->taskState)
    {
        ...
        case DRV_SDHC_TASK_MEDIA_INIT:
            {
                ...
                {
                    /* Cannot initialize this card. */
                    /* TODO: Indicate that card initialization was attempted but failed. */
                    dObj->taskState = DRV_SDHC_TASK_WAIT_FOR_DEVICE_ATTACH;
                }
                break;
            }
        ...
        case DRV_SDHC_TASK_PROCESS_QUEUE:
            {
                currentBufObj = dObj->currentBufObj = dObj->queue;
        ...
        case DRV_SDHC_TASK_SETUP_XFER:
            {
                ...
                sdhostSetupDma (&currentBufObj->buffer[0], currentBufObj->nBlocks << 9);
        ...
        case DRV_SDHC_TASK_ERROR:
                /* TODO: Error state. See if recovery can be done from this state. */

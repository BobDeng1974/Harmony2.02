 ...
 * TODO: Provide provision to transfer upto 1 MB of data. So define 16 such
 * descriptors. */
void sdhostSetupDma
(
    uint8_t *buffer,
...
)
{
    ...
    if (numBytes > 65536)
    {
        numDescLines = numBytes >> 16;
        residue = numBytes - (numDescLines << 16);
        if (residue)
        {
            numDescLines ++;
        }
    ...
    //gSDHOSTDmaDescrTable[0].attribute = (SDHOST_DESC_TABLE_ATTR_XFER_DATA | SDHOST_DESC_TABLE_ATTR_VALID | SDHOST_DESC_TABLE_ATTR_END);
...
)
{
    ...
    while ((SDHOST->clockCtl & SDHOST_CLOCK_INTERNAL_CLK_STABLE) == 0)
    {
        sdhostDelay (10);
    }
...
)
{
    ...
    if (intFlags & SDHOST_CARD_INS_REM_INTERRUPT)
    {
        if (intFlags & SDHOST_CARD_INSERTION_INTERRUPT)
        {
            /* Card insertion interrupt has occurred. */
        }

        if (intFlags & SDHOST_CARD_REMOVAL_INTERRUPT)
        {
            /* Card removal interrupt has occurred. */
        }

        /* Card interrupt bit is read only and it will be cleared only when the cause is addressed. */
    }
...
static void sdhostSetTransferMode
(
    uint8_t opcode
)
{
...
)
{
    ...
    interrupt = (SDHOST_CARD_INS_REM_INTERRUPT | SDHOST_COMMAND_COMPLETE_INTERRUPT | SDHOST_ERROR_INTERRUPT);
    if (dataPresent)
    {
    ...
    if (cardCtxt->isAttached)
    {
        interrupt |= (SDHOST_CARD_REMOVAL_INTERRUPT);
    }
    else
    {
        interrupt |= (SDHOST_CARD_INSERTION_INTERRUPT);
    }
...
static void sdhostInitVariables
(
    SDHOST_CARD_CTXT *cardCtxt
)
{
...
)
{
    ...
    /* TODO: Timeout clock to be used for configuring the timeout counter value. */
    ...
    *cardCtxt = (SDHOST_CARD_CTXT *)&gSDHOSTCardCtxt;
	...
	/* TODO: Configure the data timeout counter value. */
     ...
     * Card Det sig selection = 1 */
    SDHOST->hostCtl = SDHOST_HCTL_CARD_DET_SIG_SEL | SDHOST_HCTL_DMA_EN_CTRL;

...
void __ISR(_ADC_DATA0_VECTOR, ipl3AUTO) _IntHandlerDrvAdc(void)
{
    ...
    MC_EXTERNAL_OPAMP_APP_MC_Clarke();
   
    MC_EXTERNAL_OPAMP_APP_MC_Park();
    
    MC_EXTERNAL_OPAMP_APP_MC_Estim();
       
    // Calculate control values
    MC_EXTERNAL_OPAMP_APP_MC_DoControl();
  

    MC_EXTERNAL_OPAMP_APP_MC_CalculateParkAngle();

    /* if open loop */
    if(MC_EXTERNAL_OPAMP_APP_MC_CONTROL.bit.OpenLoop == 1)
    {
        if(MC_EXTERNAL_OPAMP_APP_MC_CONTROL.bit.Btn2Pressed == 0)
        {
    ...
    MC_EXTERNAL_OPAMP_APP_MC_InvPark();

    MC_EXTERNAL_OPAMP_APP_MC_CalcRefVec();
  
     
    MC_EXTERNAL_OPAMP_APP_MC_CalcSVGen();
    ...
    IFS3bits.AD1D0IF = 0;  
MC_EXTERNAL_OPAMP_APP_MC_CONTROL.bit.Btn2Debounce = 1;
...
void __ISR(_PWM1_VECTOR, ipl4AUTO) _IntHandlerPWM_1_Instance0(void)
{
    ...
    mc_external_opamp_appData.state = MC_EXTERNAL_OPAMP_APP_MC_STATE_STOP;

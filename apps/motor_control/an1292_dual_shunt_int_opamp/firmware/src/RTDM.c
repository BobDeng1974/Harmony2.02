/*******************************************************************************
 * Real Time Data Monitoring Protocol

  File Name:
    RTDM.c

  Summary:
    Contains the source code for RTDM based communication protocol 

  Description:
    This file contains the source code for RTDM protocol which communicates with
    the DMCI plug-in to continuously monitor variables in real-time.
...
int RTDM_Start()
{
	...
	RTDM_UART_STA_VALUE  = UART_INT_TX_BUF_EMPTY  & UART_IrDA_POL_INV_ZERO                  
	            & UART_RX_ENABLE & UART_SYNC_BREAK_DISABLED & UART_TX_ENABLE & UART_INT_RX_CHAR & 
...
int RTDM_ProcessMsgs()
{
   ...
   // This logic was added as it was observed that sometimes the EndMsgPointer was < startMSg Pointer
	...
	if((RTDMProcessMsgsTemp1 == (unsigned)RTDMcrcTempH) && (RTDMProcessMsgsTemp2 == RTDMcrcTempL))
	  {
		...
		//RTDMpu32AddressTemp = ((unsigned long *) RTDMRxBufferStartMsgPointer);
            ...
            RTDMpu32AddressTemp4 = RTDMpu32AddressTemp4 | 0x80000000;
            ...
            //RTDMpucRdData = *RTDMpu32AddressTemp;
		  ...
		  case 'M':
		    ...
		    //Capture address as 32 bit quantity to match protocol definition. 
		   //RTDMpu32AddressTemp = (unsigned long *) RTDMRxBufferStartMsgPointer;
		   // RTDMpu32AddressTemp = (RTDMRxBufferStartMsgPointer);
            RTDMpu32AddressTemp1 = *(RTDMRxBufferStartMsgPointer);
			RTDMpu32AddressTemp2 = *(RTDMRxBufferStartMsgPointer+1);
            RTDMpu32AddressTemp2 = RTDMpu32AddressTemp2 << 8;
            RTDMpu32AddressTemp3 = *(RTDMRxBufferStartMsgPointer+2);
           ...
           RTDMRxBufferStartMsgPointer += sizeof(unsigned long);
            ...
            //RTDMpucWrAddr = *RTDMpu32AddressTemp;
...
void __attribute__ ((vector(_UART2_RX_VECTOR), interrupt(IPL4AUTO), nomips16)) U2RX_DONE (void)
...
/* This is UART2 receive ISR without polling RTDM Messages */ 
 void __attribute__ ((vector(_UART2_RX_VECTOR), interrupt(IPL1AUTO), nomips16)) U2RX_DONE (void)
{
...
*			specify wich UART module is going to be used by RTDM	
*******************************************************************************/
unsigned int __attribute__((optimize("O0"))) RTDM_CumulativeCrc16(unsigned char *buf, unsigned int bsize, unsigned int crcSeed)
{

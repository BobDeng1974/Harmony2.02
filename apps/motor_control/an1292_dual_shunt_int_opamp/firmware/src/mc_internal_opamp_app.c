...
void MC_INTERNAL_OPAMP_APP_CLASSB_Tasks (void )
{
    /* Check the application's current state. */
    switch ( mc_internal_opamp_appData.classBState )
    {
        /* Application's initial state. */
        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_INIT:
        {
			...
			mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_FLASH_CRC_TEST;
            
            break;
        }

        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_FLASH_CRC_TEST:
        {
            ...
            {
				mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CHECKER_BOARD_RAM_TEST;
            }
			else
            { 
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_FLASH_CRC_TEST;
        ...
        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CHECKER_BOARD_RAM_TEST :
        {
			if(CLASSB_RAMCheckerBoardTest(ramTestStartAddress, ramTestSize)==CLASSB_TEST_PASS)
			{
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_RAM_MARCHB_TEST;
			}
			else
            { 
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CHECKER_BOARD_RAM_TEST;
        ...
        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_RAM_MARCHB_TEST:
        {
			if(CLASSB_RAMMarchBTest(ramTestStartAddress, ramTestSize )==CLASSB_TEST_PASS)
			{
				mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_RAM_MARCHC_TEST;
			}
			else	
            { 
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_RAM_MARCHB_TEST;
        ...
        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_RAM_MARCHC_TEST:
        {
			if(CLASSB_RAMMarchCTest(ramTestStartAddress, ramTestSize )==CLASSB_TEST_PASS)
			{
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_RAM_MARCHC_STACK_TEST;
			}
			else
			{ 
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_RAM_MARCHC_TEST;
        ...
        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_RAM_MARCHC_STACK_TEST:
        {
			if(CLASSB_RAMMarchCStackTest(ramTestStartAddress, ramTestSize)==CLASSB_TEST_PASS)
			{
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CLOCK_TEST;
			}
			else
            { 
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_RAM_MARCHC_STACK_TEST;
        ...
        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CLOCK_TEST:
        {
			if(CLASSB_ClockTest(SYS_CLK_FREQ , CLOCK_TEST_REFERENCE_FREQ, 1, 10) ==CLASSB_TEST_PASS)
			{
				mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CPU_PC_TEST;
			}
			else
            { 
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CLOCK_TEST;
        ...
        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CPU_PC_TEST:
        {
			if(CLASSB_CPUPCTest() == CLASSB_TEST_PASS)
			{
                mc_internal_opamp_appData.classBState =  MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CPU_REGISTERS_TEST;
			}
			else
            { 
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CPU_PC_TEST;
        ...
        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_CPU_REGISTERS_TEST:
        {
			if(CLASSB_CPURegistersTest() == CLASSB_TEST_PASS)
			{
                mc_internal_opamp_appData.classBState =  MC_INTERNAL_OPAMP_APP_CLASSB_STATE_TEST_PASS;
			}
			else
            { 
                mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_TEST_FAIL;
            }
            break;
        }
        
        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_TEST_PASS:
        {
        ...
        case MC_INTERNAL_OPAMP_APP_CLASSB_STATE_TEST_FAIL:
        {
...
// *****************************************************************************
// *****************************************************************************
void MC_INTERNAL_OPAMP_APP_PWM_Initialize(void)
{
    PTCON = 0x0000;   
...
void MC_INTERNAL_OPAMP_APP_PWM_Enable(void)
{
    PTCON=0x8000;
}

void MC_INTERNAL_OPAMP_APP_PWM_Disable(void)
{
    PTCON = 0x0000;
}

void MC_INTERNAL_OPAMP_APP_DAC_Initialize(void)
{
...
// *****************************************************************************
// *****************************************************************************
void MC_INTERNAL_OPAMP_APP_MC_Estim(void)
{
	float tempqVelEstim;
...
void MC_INTERNAL_OPAMP_APP_MC_InitEstimParm(void)  
{
...
// *****************************************************************************
// *****************************************************************************
void MC_INTERNAL_OPAMP_APP_MC_InvPark(void)
{
...
void MC_INTERNAL_OPAMP_APP_MC_CalcRefVec(void) // rotates the motor in clockwise direction
{
...
void MC_INTERNAL_OPAMP_APP_MC_InitControlParameters(void)
{
	// PI D Term     
    ...
    MC_INTERNAL_OPAMP_APP_MC_InitPI(&PIParmD);
    ...
    MC_INTERNAL_OPAMP_APP_MC_InitPI(&PIParmQ);
    ...
    MC_INTERNAL_OPAMP_APP_MC_InitPI(&PIParmQref);
	return;
}

void MC_INTERNAL_OPAMP_APP_MC_InitPI( tPIParm *pParm)
{
    pParm->qdSum = 0;
    pParm->qOut = 0;
}

void MC_INTERNAL_OPAMP_APP_MC_CalcPI( tPIParm *pParm)
{
    float Err;
    float U;
    float Exc;
...
void MC_INTERNAL_OPAMP_APP_MC_SinCos(void)
{
    // IMPORTANT:
...
void MC_INTERNAL_OPAMP_APP_MC_CalcTimes(void)
{
...
void MC_INTERNAL_OPAMP_APP_MC_CalcSVGen( void )
{
    if( SVGenParm.Vr1 >= 0 )
    {       
		// (xx1)
        if( SVGenParm.Vr2 >= 0 )
        {
            ...
            T1 = SVGenParm.Vr2;
            T2 = SVGenParm.Vr1;
            MC_INTERNAL_OPAMP_APP_MC_CalcTimes();
            dPWM1 = Ta;
            dPWM2 = Tb;
            dPWM3 = Tc;
        }
...
void MC_INTERNAL_OPAMP_APP_MC_CalculateParkAngle(void)
{
    // If open loop
	if(MC_INTERNAL_OPAMP_APP_MC_CONTROL.bit.OpenLoop)	
	{
		...
		else // switch to closed loop
		{
            ...
            MC_INTERNAL_OPAMP_APP_MC_CONTROL.bit.ChangeMode = 1;
            MC_INTERNAL_OPAMP_APP_MC_CONTROL.bit.OpenLoop = 0;
#endif
		}
...
void MC_INTERNAL_OPAMP_APP_MC_DoControl( void )
{
    ...
    if( MC_INTERNAL_OPAMP_APP_MC_CONTROL.bit.OpenLoop )
    {
        ...
        if( MC_INTERNAL_OPAMP_APP_MC_CONTROL.bit.ChangeMode )
        {
            // just changed to openloop
            MC_INTERNAL_OPAMP_APP_MC_CONTROL.bit.ChangeMode = 0;
            // synchronize angles
	...
	{ 
    	...
    	MC_INTERNAL_OPAMP_APP_MC_CalcPI(&PIParmQref);
...
void MC_INTERNAL_OPAMP_APP_MC_Tasks (void )
{
    /* Check the application's current state. */
    switch ( mc_internal_opamp_appData.mcState )
    {
        /* Application's initial state. */
        case MC_INTERNAL_OPAMP_APP_MC_STATE_INIT:
        {
            if(!BTN1)
            {
                 MC_INTERNAL_OPAMP_APP_MC_CONTROL.bit.Btn1Pressed = 1;
            }
   
            if((mc_internal_opamp_appData.classBState == MC_INTERNAL_OPAMP_APP_CLASSB_STATE_TEST_PASS) && (MC_INTERNAL_OPAMP_APP_MC_CONTROL.bit.Btn1Pressed == 1))
            {
				MC_INTERNAL_OPAMP_APP_MC_CONTROL.bit.OpenLoop = 1;
				MC_INTERNAL_OPAMP_APP_MC_CONTROL.bit.ChangeMode = 1;
				MC_INTERNAL_OPAMP_APP_MC_InitControlParameters();
				MC_INTERNAL_OPAMP_APP_MC_InitEstimParm();
				SincosParm.Angle = 0;
				SVGenParm.PWMPeriod = (float)MAX_DUTY;
				MC_INTERNAL_OPAMP_APP_PWM_Enable();
				mc_internal_opamp_appData.mcState = MC_INTERNAL_OPAMP_APP_MC_STATE_NOP;
            }
			
            break;
        }
		
        case MC_INTERNAL_OPAMP_APP_MC_STATE_NOP:
        {
            //Do Nothing in this state, ADC ISR will run the control algorithm
            break;
        }
               
        case MC_INTERNAL_OPAMP_APP_MC_STATE_STOP:
        {
            MC_INTERNAL_OPAMP_APP_PWM_Disable();
            break;
        }
...
/*******************************************************************************
  Function:
    void MC_INTERNAL_OPAMP_APP_Initialize ( void )

  Remarks:
    See prototype in mc_internal_opamp_app.h.
 */

void MC_INTERNAL_OPAMP_APP_Initialize ( void )
{
    /* Place the App state machine in its initial state. */
    mc_internal_opamp_appData.state = MC_INTERNAL_OPAMP_APP_STATE_INIT;
    //mc_internal_opamp_appData.classBState = MC_INTERNAL_OPAMP_APP_CLASSB_STATE_INIT;
	/* Initialize PWM */
	MC_INTERNAL_OPAMP_APP_PWM_Initialize();
	
	/* Initialize DAC */
	MC_INTERNAL_OPAMP_APP_DAC_Initialize();
...
/******************************************************************************
  Function:
    void MC_INTERNAL_OPAMP_APP_Tasks ( void )
...
void MC_INTERNAL_OPAMP_APP_Tasks ( void )
{
    ...
    /* Check the application's current state. */
    switch ( mc_internal_opamp_appData.state )
    {
        /* Application's initial state. */
        case MC_INTERNAL_OPAMP_APP_STATE_INIT:
        {
            ...
            if (appInitialized)
            {
                ...
                mc_internal_opamp_appData.state = MC_INTERNAL_OPAMP_APP_STATE_SERVICE_TASKS;
            }
            break;
        }

        case MC_INTERNAL_OPAMP_APP_STATE_SERVICE_TASKS:
        {
			MC_INTERNAL_OPAMP_APP_CLASSB_Tasks();
			MC_INTERNAL_OPAMP_APP_MC_Tasks();

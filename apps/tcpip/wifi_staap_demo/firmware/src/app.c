...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state )
    {
		...
		flash_open_status =  WIFI_initialization();
              ...
              {              	
				appData.state = APP_FS_MOUNT_DISK;
              }
            break;
        }
	 case APP_FS_MOUNT_DISK:
		{
            //if(SYS_FS_Mount(SYS_FS_NVM_VOL, LOCAL_WEBSITE_PATH_FS, MPFS2, 0, NULL)  == 0)
            if(SYS_FS_Mount("/dev/rama1", LOCAL_WEBSITE_PATH_FS, MPFS2, 0, NULL)  == 0)
            {
